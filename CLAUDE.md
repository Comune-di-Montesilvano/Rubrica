...
Docker image: `ghcr.io/comune-di-montesilvano/rubrica` (lowercase — GHCR/OCI require lowercase image names, unlike the GitHub org/repo name itself). Binary/container/system-user name: `rubrica`. Database file path is unchanged (`/data/ldavsync.db`, see `DATABASE_PATH` in `docker-compose.yml`) — deliberately not renamed, to avoid touching the volume path of any already-running deployment. **`/data` is a named Docker volume (`rubrica-data`), not a bind mount** — Portainer git-stack deploys clone the repo into a per-commit path it doesn't own, and a bind mount there (`./data:/data`) fails with "permission denied" trying to create the mountpoint. Switching a running bind-mount deployment to the named volume starts with an empty DB (LDAP/PBX resync repopulates contacts, but local-only state — admin config, manual contacts, groups, areas — is lost and must be reconfigured or manually `docker cp`'d in beforehand). **File is named `docker-compose.yml`, not `compose.yml`** — the Portainer git-stack deploy expects that exact filename and fails with "no such file or directory" otherwise. **No `env_file` directive** — a Portainer git-stack clone has no `.env` on disk (it's gitignored, holds real secrets), so an `env_file` entry fails the pull with "env file ... not found". Instead every var is listed explicitly under `environment:` as `${VAR}` — Compose's built-in `${VAR}` substitution reads a local `.env` for `docker compose up` (unchanged dev flow) and reads the process environment on Portainer, where the stack's own "Environment variables" UI sets it.
...
- **`internal/pbx`**: screen-scraping (nessuna API ufficiale) della web console del centralino Invidea "ViVo" — sync orario opzionale, stesso giro di `ldap.SyncContacts`, attivo solo se configurato da `/admin/pbx` (nessuna env var: URL/utente/password vivono solo in `app_config`). **A logged-in session still can't hit a module URL (`vivo.index.php?module=...`) directly** — it bounces to a blank frame-buster page unless the request's `Referer` is the exact "_view" landing page that precedes it in the real frameset (`sip&mode=peers_view` → `monitor&mode=peers`, `extensions&mode=callgroups_view` → `extensions&mode=callgroups`); `Client.getWithLanding` replicates this two-hop navigation — don't call the module endpoints with a bare `Referer: vivo.php`. Peers SIP non coperti da un contatto `source='ldap'` (stesso `ldap_ext`, qualunque `disabled`) diventano `contacts` `source='pbx'`. Call group del centralino estendono `group_numbers` (colonne `source`/`name_override`/`area`): il nome è overridabile in locale, ma il mapping membri è sempre sovrascritto dal centralino. `/admin/pbx` mostra anche diagnostica calcolata dall'ultimo sync riuscito (`pbx.SyncResult`): interni con nome corrispondente ma mancante da un lato, nomi disallineati sullo stesso interno, "interni riciclabili" (stesso interno+nome, ma il titolare in AD è `disabled`), e — `pbx.FindDisabledGroupMembers` — membri di un gruppo di chiamata il cui interno corrisponde a un contatto di dominio `disabled` (il centralino resta comunque fonte di verità sull'appartenenza al gruppo: il membro NON sparisce da `group_numbers`/`group_members`, viene solo segnalato). **Distinto dal filtro di visualizzazione**: `phonebook.GroupWithMembers.ActiveMembers()` nasconde i membri `disabled` solo nella rubrica pubblica (`search_results.html`) — le pagine admin (`/admin/groups`, gestione membri) continuano a mostrare `.Members` non filtrato, serve vedere anche i disattivi per poterli rimuovere. Vedi `docs/superpowers/specs/2026-09-13-pbx-scraping-design.md`.
...
- **`docker-compose.yml` has `pull_policy: always`** on the `rubrica` service — needed because it also has a `build:` section (for the local `docker compose up -d --build` dev flow), and without it a plain `docker compose up -d` (what Portainer's stack redeploy runs) can use a stale local build/cache instead of pulling the freshly-pushed `ghcr.io/.../rubrica:latest`. Without this, the UI version tag shows `dev` in prod even though `release.yml` bakes the real tag into the pushed image (`AppVersion` via ldflags, see `cmd/server/main.go`) — a local build with no `VERSION` build-arg silently defaults to `dev`.
